...
import { semesterSchema } from "@/lib/validators/admin";
...
export async function GET(request: NextRequest) {
  try {
    ...
    return errorResponse(new AppError("INTERNAL_SERVER_ERROR", "Error obteniendo semestres.", 500));
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { code, name, startDate, endDate, isCurrent } = result.data;
    ...
    if (error instanceof AppError) return errorResponse(error);
    return errorResponse(new AppError("INTERNAL_SERVER_ERROR", "Error creando semestre.", 500));
  }
}
