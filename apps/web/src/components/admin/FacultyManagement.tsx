...
import { useFaculties } from "@/hooks/useAdmin";
import { Plus, Edit2, Trash2, Building2 } from "lucide-react";

export function FacultyManagement() {
  const { data, loading, error, createFaculty, deleteFaculty } = useFaculties();
  ...
  const handleDelete = async (id: number, name: string) => {
    if (window.confirm(`¿Estás seguro de eliminar la facultad ${name}? Esta acción no se puede deshacer.`)) {
      try {
  ...
  if (loading) return <div className="p-8 text-center text-slate-500 animate-pulse">Cargando facultades...</div>;
      ...
      {isCreating && (
        <form onSubmit={handleCreate} className="p-6 border-b border-slate-100 bg-indigo-50/50">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Nombre de la Facultad</label>
              <input 
                type="text" 
                required 
                className="w-full rounded-md border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm px-3 py-2 border"
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left text-sm whitespace-nowrap">
          ...
          <tbody className="divide-y divide-slate-100 bg-white">
            {!data || data.length === 0 ? (
              <tr>
                <td colSpan={3} className="px-6 py-8 text-center text-slate-500">No hay facultades registradas</td>
              </tr>
            ) : (
              data.map((faculty) => (
                <tr key={faculty.id} className="hover:bg-slate-50">
                  <td className="px-6 py-4 font-mono text-xs text-slate-600">{faculty.code}</td>
                  <td className="px-6 py-4 font-medium text-slate-800">{faculty.name}</td>
